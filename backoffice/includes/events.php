			...
			<?php
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						$result = $db->query("SELECT title FROM events WHERE start_date = '".$currentDate."' AND status = 1");
...
function getEvents($date = ''){
	...
	$result = $db->query("SELECT title FROM events WHERE start_date = '".$date."' AND status = 1");
...
function addEvent($date,$title){
	...
	$insert = $db->query("INSERT INTO events (title,start_date,created,modified) VALUES ('".$title."','".$date."','".$currentDate."','".$currentDate."')");
	if($insert){
		echo 'ok';
	}else{
		echo 'err';
